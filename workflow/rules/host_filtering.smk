...
rule cleanup_fastp:
    input:
        # dependency only
        fastqc=rules.fastqc.output.zip1,
        map_human=rules.map_to_human.output.bam,
    ...
    log:
        "logs/{project}/cleanup_fastp/{sample}.log",
    conda:
        "../envs/unix.yaml"
    threads: 1
    priority: 1
...
rule cleanup_fastq:
    input:
        # dependency only
        gz_reads=get_filtered_gz_fastqs,

        # actually delete these
        to_delete=[
            rules.filter_human.output.filtered
        ]
    output:
        touch("results/{project}/cleanup/{sample}_fastq.done"),
    log:
        "logs/{project}/cleanup_fastq/{sample}.log",
    conda:
        "../envs/unix.yaml"
    threads: 1
    priority: 1
...
rule cleanup_human_map:
    input:
        ...
        to_delete=[
            rules.map_to_human.output.bam,
            rules.index_human_alignment.output.bai
        ]
    output:
        touch("results/{project}/cleanup/{sample}_human_map.done"),
    log:
        "logs/{project}/cleanup_human_map/{sample}.log",
    conda:
        "../envs/unix.yaml"
    threads: 1
    priority: 1
